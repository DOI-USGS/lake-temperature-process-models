...
#' For the string extraction to work, data_tile_no must be at the END of the meteo filename
#' @param meteo_feathers - names of the gcm/time-period/tile specific feather files
...
build_model_config <- function(meteo_feathers, lake_cell_tile_xwalk, gcm_names, gcm_dates){
  ...
  data_tile_no_list <- paste(unique(lake_cell_tile_xwalk$data_tile_no),collapse= "|")
  ...
  meteo_branches <- tibble(
    ...
    data_tile_no = as.numeric(str_extract(tools::file_path_sans_ext(meteo_fl), paste0(data_tile_no_list, "$")))
  )
  ...
  model_config <- tidyr::expand_grid(
    nesting(select(lake_cell_tile_xwalk, site_id, state, data_cell_no, data_tile_no)),
    gcm = gcm_names, 
    time_period = gcm_dates) %>%
    arrange(site_id) %>%
    left_join(meteo_branches, by=c('gcm', 'data_tile_no', 'time_period')) %>%
    select(-data_tile_no) %>%
    rowwise()
  return(model_config)
}
