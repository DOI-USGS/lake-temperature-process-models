...
plot_20yr_preds <- function(model_run) {
  glm_preds <- arrow::read_feather(sprintf('2_run/out/Denali/%s',model_run$output_file)) %>%
  ...
  # return(doy_plot)
  plot_filepath <- sprintf('4_visualize/out/20yr_preds_ice_%s_%s.png', model_run$lake_id, model_run$gcm)
